...
// ACCESSIBILITY: Setup basic accessibility features
function setupAccessibility() {
    // Add keyboard support
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Enter' || e.key === ' ') {
            if (e.target === askButton && !askButton.disabled) {
                e.preventDefault();
                handleAskUniverse();
            } else if (e.target === askAgainButton) {
                e.preventDefault();
                resetToFocus();
            } else if (e.target === shareButton) {
                e.preventDefault();
                generateAndDownloadImage();
            }
        }
        
        // Escape to return to focus state
        if (e.key === 'Escape' && !messageState.classList.contains('hidden')) {
            resetToFocus();
        }
    });
    
    // Announce to screen readers (simple version)
...
function initializeApp() {
    ...
    // FIXED: Logo click should stay on same page, not redirect
    const logoLink = document.querySelector('.logo-link');
    logoLink.addEventListener('click', function(e) {
        e.preventDefault();
        if (!focusState.classList.contains('hidden')) {
            // Already on focus state, just restart
            startFocusSequence();
        } else {
...
// FIXED: Restored original focus sequence behavior
function startFocusSequence() {
    ...
    if (window.announceToScreenReader) {
        window.announceToScreenReader('Focus preparation beginning. Hold your question in mind.');
    }
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            // Announce readiness
            if (window.announceToScreenReader) {
                window.announceToScreenReader('Ready to receive your cosmic message.');
            }
        }, 3000);
    }, 500);
}
...
function displayMessage(message) {
    ...
    setTimeout(() => {
        ...
        if (window.announceToScreenReader) {
            window.announceToScreenReader(`The Universe says: ${message}`);
        }
    }, 50);
...
// FIXED: Share functionality - removed positioning bug and filename issue
...
function generateRestOfImage() {
    ...
    // FIXED: Download with original filename format (removed _cosmic_message)
    canvas.toBlob(function(blob) {
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        
        // FIXED: Generate timestamp filename without _cosmic_message
        const now = new Date();
